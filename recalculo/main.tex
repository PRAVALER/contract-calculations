...
\section{Consolidação e atualização dos contratos}{
  ...
  \subsection{Cálculo do CET ao mês}{
    ...
    \subsubsection{Preparação conceitual do processo}{
      ...
      Conjunto dos "parciais" $PA$, com $pa_{j ^ \prime}$ como elemento:
      \begin{equation*}
        PA = \{pa_{1}, \cdots, pa_{l}\} \text{, onde } j ^ \prime = \{1, \cdots, l\}
      \end{equation*}

      Conjunto dos juros $T$, com $t_{j}$ como elemento:
      \begin{equation*}
        T = \{t_{1}, \cdots, t_{m}\} \text{, onde } j = \{1, \cdots, m\}
      \end{equation*}
    }

    \subsubsection{Cálculo inicial do parcial}{
      O cálculo do parcial inicial seguirá a definição vista na fórmula (\ref{eq:limitVF})
      \ref{subsec:Alice}
    ...
    \subsubsection{Cálculo do juros}{
      ...
      juros é dependente do parcial tendendo ao valor financiado.
      \begin{equation}
        s
      \end{equation}
    }
